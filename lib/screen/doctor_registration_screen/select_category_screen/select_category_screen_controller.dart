...
class SelectCategoryScreenController extends GetxController {
  ...
  SelectCategoryScreenController(this.screenType);
  ...
  void onSearchCategories(String value) {
    if (value.isEmpty) {
      ...
      filterList = categories.where((element) => element.title!.isCaseInsensitiveContains(value)).toList();
    }
    update();
  }
  ...
  void onSuggestUsTap() {
    Get.bottomSheet(
      SuggestUsSheet(controller: SelectCategoryScreenController(screenType)),
  ...
  void updateDoctorDetailsApiCall() {
    if (selectedCategories == null) {
      CustomUi.snackBar(message: S.current.pleaseSelectCategory);
      return;
    }
    CustomUi.loader();
    ApiService.instance.updateDoctorDetails(categoryId: selectedCategories?.id).then((value) {
  ...
  void navigateRoot() {
    ...
    } else if (userData?.aboutYouself == null || userData?.educationalJourney == null) {
  ...
  void suggestDoctorCategoryApiCall() {
    if (titleController.text.isEmpty) {
      CustomUi.snackBar(message: S.current.addCategoryNameForSuggestion);

      return;
    }
    if (aboutController.text.isEmpty) {
      CustomUi.snackBar(message: S.current.pleaseExplainBrieflyEtc);
      return;
    }
    CustomUi.loader();
    ApiService.instance.suggestDoctorCategory(title: titleController.text, about: aboutController.text).then(
      (value) {
        if (value.status == true) {
          Get.back();
          Get.back();
