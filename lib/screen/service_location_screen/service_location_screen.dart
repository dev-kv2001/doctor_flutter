...
class ServiceLocationScreen extends StatelessWidget {
    ...
    return Scaffold(
      ...
      body: Column(
        children: [
          ...
          SettingsFiledTopBar(title1: S.current.addHospitalsWhereYouEtc, title2: S.current.thisHelpsPatientsToEtc),
          GetBuilder(
            init: controller,
            builder: (context) {
              return controller.isLoading ? CustomUi.loaderWidget() : _serviceLocationList(controller: controller);
            },
          ),
        ],
      ),
  ...
  Widget _serviceLocationList({required ServiceLocationScreenController controller}) {
    return GetBuilder(
      init: controller,
      builder: (context) {
        return Expanded(
          child: controller.serviceLocation == null || controller.serviceLocation!.isEmpty
              ? CustomUi.noData(message: '${S.current.no} ${S.current.servicesLocation}')
              : ListView.builder(
...
class _PopUpMenuCustomState extends State<PopUpMenuCustom> {
  ...
  Widget build(BuildContext context) {
    return PopupMenuButton(
      ...
      onSelected: (value) {
        widget.controller.hospitalNameController = TextEditingController(text: widget.data?.hospitalTitle ?? '');
        widget.controller.hospitalAddressController = TextEditingController(text: widget.data?.hospitalAddress ?? '');

        if (value == 0) {
          widget.controller.navigatePage(apiType: 2, serviceLocations: widget.data);
        } else {
