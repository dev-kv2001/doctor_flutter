...
class AppointmentChatScreenController extends GetxController {
  ...
  void sendTextMsg() {
    if (msgController.text.isNotEmpty) {
      sendChatMessage(msgType: FirebaseRes.text, msg: msgController.text.trim());
      msgController.clear();
    }
  }
  ...
  Future<void> sendChatMessage(
    ...
    if (appointmentData?.user?.isNotification == 1) {
      ...
      ApiService().pushNotification(data: map, token: appointmentData?.user?.deviceToken ?? '');
    }
  }
  ...
  void onSendMediaTap({required String image, required int type, String? video}) async {
    if (type == 0) {
      ...
      Get.back();
      sendChatMessage(msgType: FirebaseRes.image, msg: sendMediaController.text.trim(), image: imageUrl);
    } else {
      if (videoUrl == null) {
  ...
  void onVideoCallBtnTap() {
    showDatePicker(
      ...
      lastDate: DateTime.utc(DateTime.now().year + 1, DateTime.now().month).subtract(
        const Duration(days: 1),
      ),
    ).then((date) {
      if (date != null) {
        showTimePicker(context: Get.context!, initialTime: TimeOfDay.now()).then((value) {
          if (value != null) {
            DateTime time = DateTime(date.year, date.month, date.day, value.hour, value.minute);
            sendChatMessage(msgType: FirebaseRes.videoCall, videoCallTime: time.millisecondsSinceEpoch.toString());
          }
        });
      }
    });
  ...
  void onJoinMeeting(AppointmentChat data) {
    if (data.videoCall?.isStarted == true) {
      DateTime meetingTime = DateTime.fromMillisecondsSinceEpoch(int.parse(data.videoCall?.time ?? ''));
      ...
      if (meetingTime.isBefore(now)) {
        ...
        if (call?.token == null || call!.token!.isEmpty) {
          CustomUi.loader();
          ApiService.instance.getAgoraToken(channelName: call?.channelId ?? '').then((t) {
            Get.back();
            if (t.status == true) {
              call?.token = t.token;
  ...
  void endVideoStatusUpdate({VideoCall? call, required bool value, required AppointmentChat data}) {
    if (value == true) {
